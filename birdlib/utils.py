...
def generate_noisy_image(pil_img, speckle_coef=0.8, speckle_lambda=0.4, gauss_coef=0.2, gauss_sigma=0.15):
...
def generate_noisy_image_and_mask(pil_img, speckle_coef=0.8, speckle_lambda=0.12, gauss_coef=0.2, gauss_sigma=0.05):
    init_image =  pil_img.resize((256, 256))
    gauss = np.random.normal(0,speckle_lambda, 3*256*256)
    gauss = gauss.reshape(256,256,3).astype(np.float32)
...
def generate_lr_image(pil_img, downsampling_ratio, speckle_coef=0.8, speckle_lambda=0.12, gauss_coef=0.2, gauss_sigma=0.05):
...
def get_conv(scale):
    ...
    kernel = np.load(os.path.join(package_dir, 'data/kernel.npy'))
    kernel_torch = torch.from_numpy(kernel).unsqueeze(0).unsqueeze(0).float()
    conv.weight = torch.nn.Parameter(kernel_torch)
    return conv

def generate_blurry_image(pil_image, kernel_size=41, speckle_coef=0.8, speckle_lambda=0.12, gauss_coef=0.2, gauss_sigma=0.05):
...
def load_pretrained_diffusion_model(config, ckpt):
    model = Model(config)
